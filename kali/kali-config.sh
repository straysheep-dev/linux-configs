...
AA_D='/etc/apparmor.d/'
AA_PATH='/etc/apparmor.d/usr.bin.firefox-esr'
AA_BROWSERS_PATH='/etc/apparmor.d/abstractions/ubuntu-browsers.d/firefox-esr'
AA_LOCAL_PATH='/etc/apparmor.d/local/usr.bin.firefox-esr'
...
function updateAppArmor() {
	...
	if [[ -e "$AA_D"/disable/"$AA_FIREFOX" ]]; then
		rm "$AA_D"/disable/"$AA_FIREFOX"
	fi

	# /etc/apparmor.d/usr.bin.firefox-esr
	if ! [ -e "$AA_PATH" ]; then
		curl -Lf 'https://raw.githubusercontent.com/straysheep-dev/linux-configs/main/apparmor/apparmor-usr.bin.firefox-esr' > "$AA_PATH"
		if ! (sha256sum "$AA_PATH" | grep "eb3f9e1632e7717d5ce2eea64cbc0af43a6131345d47946a6991300a47b5a85b  $AA_PATH"); then
			echo -e "${RED}[X]Bad checksum. Quitting...${RESET}"
			exit 1
		else
			echo -e "${GREEN}[OK]$AA_PATH${RESET}"
		fi
	fi

	# /etc/apparmor.d/abstractions/ubuntu-browsers.d/firefox-esr
	if ! [ -e "$AA_BROWSERS_PATH" ]; then
		curl -Lf 'https://raw.githubusercontent.com/straysheep-dev/linux-configs/main/apparmor/apparmor-firefox.abstractions' > "$AA_BROWSERS_PATH"
		if ! (sha256sum "$AA_BROWSERS_PATH" | grep "e712b2f363663db162c804ee57d8e3e6ef983d3f0caa2dea69974d3edc086c7e  $AA_BROWSERS_PATH"); then
			echo -e "${RED}[X]Bad checksum. Quitting...${RESET}"
			exit 1
		else
			echo -e "${GREEN}[OK]$AA_BROWSERS_PATH${RESET}"
		fi
	fi

	# /etc/apparmor.d/local/usr.bin.firefox-esr
	if ! [ -e "$AA_LOCAL_PATH" ]; then
		curl -Lf 'https://raw.githubusercontent.com/straysheep-dev/linux-configs/main/apparmor/apparmor-usr.bin.firefox.local' > "$AA_LOCAL_PATH"
		if ! (sha256sum "$AA_LOCAL_PATH" | grep "bcfa36de8bfc0f6063beb107bdf98af515f40fea53a7bca249658d21a0d2234a  $AA_LOCAL_PATH"); then
			echo -e "${RED}[X]Bad checksum. Quitting...${RESET}"
			exit 1
		else
			echo -e "${GREEN}[OK]$AA_LOCAL_PATH${RESET}"
		fi
	fi
