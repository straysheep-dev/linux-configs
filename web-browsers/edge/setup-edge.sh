...
function AddEdgeFeed() {
	...
	for file in /etc/apt/sources.list.d/microsoft-edge*
	do
		if ! [ -e "$file" ]; then
			# Including "signed-by=/etc/apt/trusted.gpg.d/microsoft.gpg" seems to cause issues, as edge writes it's own list file that will conflict with this one
			# If you run into this issue, overwrite the autogenerated list file with ours: sudo mv microsoft-edge-stable.list microsoft-edge.list
